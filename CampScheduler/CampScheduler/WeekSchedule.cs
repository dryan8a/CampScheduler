...
namespace CampScheduler
{
    public class WeekSchedule : Schedule
    {
        ...
        public void OutputGroup(Excel.Worksheet outputSheet, string[] takenSheetNames, Group group)
        {
            ...
            //find num of different times across a week
            var numOfTimes = WeekInfo.First().Value.Times.Length;

            outputRange.Range["A1", (char)('A' + numOfTimes) + "1"].Merge();
            ...
            for (int column = 0; column < numOfTimes; column++)
            {
                //currently takes times from first day
                outputRange.Cells[2, column + 2].Value2 = WeekInfo.First().Value.Times[column];
                outputRange.Cells[3, column + 2].Value2 = column + 1;
            }

            for (int row = 0; row < NumOfDays; row++)
            {
                outputRange.Cells[row + 4, 1].Value2 = WeekInfo.Keys.ElementAt(row);
            }
            ...
            for (int day = 0; day < NumOfDays; day++)
            {
                for (int column = 0; column < numOfTimes; column++)
                {
                    outputRange.Cells[day + 4, column + 2].Value2 = ScheduleData[WeekInfo.Keys.ElementAt(day)][column, group.RowNum];
                }
            }
